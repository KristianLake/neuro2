...
BEGIN
    ...
    BEGIN
      ...
      IF _admin_header IS NOT NULL THEN
        ...
        IF _updater_id IS NOT NULL THEN
          _updater_role := 'admin';
          RAISE LOG 'Using admin user ID: % with role: %', _updater_id, _updater_role;
        ELSE
          RAISE LOG 'Admin user not found or not admin for ID: %', _admin_header;
        END IF;
      END IF;
  ...
  IF (OLD.raw_user_meta_data->>'full_name') IS DISTINCT FROM (NEW.raw_user_meta_data->>'full_name') THEN
    ...
    RAISE LOG 'Inserted profile history record for user: % by updater: % with role: %',
      _user_id, _updater_id, _updater_role;
  END IF;
