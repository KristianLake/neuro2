/*
  # Improve User Tracking Implementation

  1. Changes
    - Improve admin user tracking
...
BEGIN
  ...
  IF _jwt_role = 'service_role' THEN
    ...
    BEGIN
      ...
      IF _admin_header IS NOT NULL AND 
        ...
        SELECT id, role INTO _updater_id, _updater_role
        FROM public.users
  ...
  IF OLD.raw_user_meta_data IS DISTINCT FROM NEW.raw_user_meta_data THEN
    ...
    -- Track other metadata changes here
    -- Add more IF blocks for other fields as needed
  END IF;
  ...
  RAISE LOG 'Profile update - User: %, Updater: % (%), Auth: %',
    _user_id,
    _updater_id,
    _updater_role,
    CASE WHEN _jwt_role = 'service_role' THEN 'admin' ELSE 'user' END;
