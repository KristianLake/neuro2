...
-- Create type for purchase status if it doesn't exist
DO $$ BEGIN
...
-- Add version column first
ALTER TABLE purchases 
ADD COLUMN IF NOT EXISTS version integer DEFAULT 1;

-- Create temporary column for status conversion
ALTER TABLE purchases 
ADD COLUMN IF NOT EXISTS new_status purchase_status;

-- Convert existing statuses
UPDATE purchases 
SET new_status = status::purchase_status;
...
-- Make new status column not null
ALTER TABLE purchases 
...
CREATE OR REPLACE FUNCTION update_purchase_status(
  p_purchase_id uuid,
  p_user_id uuid,
  p_current_status purchase_status,
  p_new_status purchase_status,
  p_version integer
)
RETURNS boolean AS $$
DECLARE
...
-- Recreate course access trigger
...
GRANT EXECUTE ON FUNCTION update_purchase_status TO authenticated;
