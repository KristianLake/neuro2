/*
  ...
  1. Changes
    - Improve service role handling
    - Fix admin header processing
...
BEGIN
  ...
  BEGIN
    ...
    RAISE LOG 'Request headers: %', _headers;
  EXCEPTION WHEN OTHERS THEN
    RAISE DEBUG 'Error getting headers: %', SQLERRM;
  END;
  ...
  IF _jwt_role = 'service_role' THEN
    ...
    BEGIN
      ...
      _admin_header := _headers->>'x-admin-user-id';
      ...
      IF _admin_header IS NOT NULL AND 
        ...
        ELSE
          RAISE LOG 'Admin user not found or not admin: %', _admin_header;
          ...
          IF _updater_id IS NOT NULL THEN
            _updater_role := 'admin';
            RAISE LOG 'Fallback to first admin: %', _updater_id;
          END IF;
        END IF;
      ELSE
        RAISE LOG 'Invalid or missing admin header: %', _admin_header;
        -- Fallback to first admin user
        SELECT id INTO _updater_id
        FROM public.users
        WHERE role = 'admin'
      ...
      END IF;
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG 'Error processing admin header: %', SQLERRM;
      -- Fallback to first admin user
      SELECT id INTO _updater_id
      FROM public.users
      WHERE role = 'admin'
      ...
      IF _updater_id IS NOT NULL THEN
        _updater_role := 'admin';
        RAISE LOG 'Fallback to first admin after error: %', _updater_id;
      END IF;
    END;
  ELSE
