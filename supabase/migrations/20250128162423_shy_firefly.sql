  ...
  IF NEW.status = 'completed' AND OLD.status != 'completed' THEN
    ...
    BEGIN
      ...
      IF product_type = 'module' THEN
        ...
        -- Also insert a record for the full course ID
        INSERT INTO course_access (user_id, module_id)
        VALUES (NEW.user_id, NEW.product_id)
        ON CONFLICT (user_id, module_id) DO NOTHING;
...
BEGIN
  RETURN EXISTS (
    ...
    AND (
      -- Direct module/course access
      ca.module_id = p_module_id
      OR 
      -- Full course access grants access to all modules
...
BEGIN
  ...
  LOOP
    -- Grant access to all modules and the course itself
    INSERT INTO course_access (user_id, module_id)
    SELECT 
      r.user_id,
      module_id
